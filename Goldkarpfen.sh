...
__ARCHIVE(){
  ...
  cd itp-files
  echo "  ## archiving"
  if TZ=UTC tar czfv "../tmp/""$OWN_ALIAS"-"$OWN_ADDR"".itp.tar.gz" --mtime="$(TZ=UTC date -d "$(date +'%FT%H:00%z')" +'%F %T')" "$OWN_ALIAS""-""$OWN_ADDR"".itp.sha512sum" "$OWN_ALIAS""-""$OWN_ADDR"".itp" "$OWN_ALIAS""-""$OWN_ADDR"".itp.sha512sum"".sig" --utc;then
