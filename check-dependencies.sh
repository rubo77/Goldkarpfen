...
set -- "ag awk basename cat cmp cp cut date dc dd diff du file fold grep gunzip gzip head ls mkdir mktemp mv openssl pgrep printf ps sed seq sha512sum sort tail tar touch tput tr uniq wc xxd"
__WHICH(){
  for PROG in $1;do
