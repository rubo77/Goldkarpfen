...
if ! which tput gzip dd du mktemp xxd ag dc openssl more fold awk sed grep basename sha512sum tr cat touch tail head cmp tar date sort uniq wc file pwd diff> /dev/null;then RE="ERROR";fi
echo $RE
