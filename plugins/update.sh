...
__USER_INQR(){
  set -- "$(printf "url1 target=termux(android)\nurl1 target=ish(ios)\nlocal target=termux(android)\nlocal target=ish(ios)\n" | pipe_if_not_empty $GK_FZF_CMD)"
  ...
  if test "$2" = "target=termux(android)";then
    ...
    qrencode "apk upgrade ; apk add $T_BUF4 && if ! pidof $T_BUF6 > /dev/null;then eval \"$T_BUF5\";fi" -t UTF8
    echo "Enter for next"; read T_BUF
    qrencode "$(echo "U=$1;$T_CMD > gki.sh && sh gki.sh -a \$U" | sed 's/\\/\\\\/')" -t UTF8
  fi
  echo -n "$(tput sgr0)"
}
