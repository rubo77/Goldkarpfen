...
if ! test -r Goldkarpfen.config; then echo "  EE the config file Goldkarpfen.config is missing!";exit 1;fi
...
#SERVER_PORT
set -- "$(sed -n '4p' Goldkarpfen.config)"
if test -x ./my-start-services.sh;then
